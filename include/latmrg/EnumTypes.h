 ...
 */
   ...
   * `MMRG`: the matrix multiple-recursive generator.
   * `MWC`: the multiply-with-carry generator. Usually transformed into an LCG. <br>
   */
  ...
  static std::string toStringGen (GenType gen) {
	    switch (gen) {
	      case LCG:
	        return "LCG";
	      case MRG:
  ...
  static std::string toStringLattice (LatticeType lat) {
      switch (lat) {
        case FULL:
          return "FULL";
        case RECURRENT:
  ...
  enum LacunaryType {NONE, SUBVECTOR, ARBITRARYINDICES };
  static std::string toStringLacunary (LacunaryType lacuna) {
	    switch (lacuna) {
	      case NONE:
	        return "NONE";
	      case SUBVECTOR:
