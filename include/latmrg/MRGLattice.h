 ...
 * (MRGs) with modulus \f$a_j\f$, order \f$a_j\f$, and vector of multipliers \f${\bf a} = (a_1, . . . , a_k)\f$.
 * In this implementation, instead of computing and storing the full basis and/or its m-dual,
 * we store only a vector \f$\bf y\f$ that contains the successive values produced by the MRG recurrence
 * when the initial values are \f$(0,\dots,0,1)\f$. The successive entries of this vector are computed
 * and saved when they are needed.  Its coordinates are sufficient to build a basis or its m-dual
