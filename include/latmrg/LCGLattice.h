...
public:
    ...
    */
   LCGLattice(const Int &n, const Int &a, int maxDim, LatticeTester::NormType norm =
    ...
    * \f]
    */
   LCGLattice(const Int &n, const Int &a, int dim, int t, LatticeTester::NormType norm =
         LatticeTester::L2NORM);
...
template<typename Int, typename Real>
LCGLattice<Int, Real>::LCGLattice(const Int &n, const Int &a, int maxDim,
...
template<typename Int, typename Real>
LCGLattice<Int, Real>::LCGLattice(const Int &n, const Int &a, int maxDim, int t,
      LatticeTester::NormType norm) :
      LatticeTester::IntLatticeExt<Int, Real>::IntLatticeExt(n, 0, maxDim, norm) {
   m_a = a;
   m_shift = t;
...
void LCGLattice<Int, Real>::buildBasis(int d) {
   ...
   for (int i = 2; i <= d; i++) {
      for (int j = 1; j <= d; j++) {
         if (i == j) this->m_basis(i, j) = this->m_modulo;
