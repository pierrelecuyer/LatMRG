...
#define ORDERMAX 100
  ...
  template<typename Int>
    void MMRGLattice<Int>::setLac(const LatticeTester::Lacunary & lac)
    {
      m_lac = lac;
      m_lacunaryFlag = true;
    }
  ...
  template<typename Int>
    string MMRGLattice<Int>::toStringGeneratorMatrix () const
    {
        ...
        if (k == maxIter) { // we completed the end of m_basis matrix
          ...
          for (int i = 0; i < sizeA; i++) {
            for (int j = 0; j < residu; j ++)
              m_basis[i][k*sizeA +j] = conv<Int>(temp[i][j]);
          }
        } else {
          for (int i = 0; i < sizeA; i++) {
    ...
    void MMRGLattice<Int>::buildLacunaryBasis (int dimension)
    {
      ...
      for (int k = 1; k < maxIter+1; k++) {
        ...
        if (k == maxIter) { // we completed the end of m_basis matrix
          ...
          for (int i = 0; i < sizeA; i++) {
            for (int j = 0; j < residu; j ++)
              tempBasis[i][k*sizeA +j] = conv<Int>(temp[i][j]);
          }
        } else {
          for (int i = 0; i < sizeA; i++) {
            for (int j = 0; j < sizeA; j ++)
  ...
  template<typename Int>
    ...
    // X_n = A X_{n-1} mod m. We have: dimension >= order.
    {
      ...
      for (int i = 0; i < (newDimension-1); i++) {
        ...
        initialState = conv<MVect>( 
            transpose(temp) * conv<MVectP>(initialState));
